...
class GPIOControlNode(Node):
    def __init__(self):
        ...
        self.service_left = self.create_service(
            SetBool, SERVICE_NAME_LEFT, self.gpio_control_callback_L
        )
        self.service_right = self.create_service(
            SetBool, SERVICE_NAME_RIGHT, self.gpio_control_callback_R
        )
    ...
    def gpio_control_callback_L(self, request, response):
        self.delay = (
            self.get_parameter(PARAMETER_NAME).get_parameter_value().integer_value
        )
